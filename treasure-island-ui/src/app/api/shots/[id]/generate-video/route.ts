...
import { queuePrompt, buildWan2_1_I2VWorkflow, buildWan2_1_I2VWorkflow_14B, buildI2VWorkflow, uploadImage, getHost } from "@/lib/comfyui";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Collect all approved image IDs (prefer new multi-array, fall back to legacy single)
  const approvedIds: string[] = shot.approved_image_ids?.length
  ...
  if (queued.length > 0) shot.status = "video_generating";
  await save(db);
